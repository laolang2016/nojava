...
namespace nami {
spdlog::level::level_enum LogUtil::global_level = AppConfig::getInstance()->logLevel();  // NOLINT
...
std::vector<spdlog::sink_ptr> LogUtil::createSinks(const std::string &log_file_name) {
    ...
    sink2->set_level(LogUtil::getGlobalLevel());
...
void LogUtil::init(spdlog::level::level_enum level, std::string log_file) {
    ...
    spdlog::flush_on(AppConfig::getInstance()->logLevel());
}
};  // namespace nami